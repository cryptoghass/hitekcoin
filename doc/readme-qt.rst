...
Alternatively, install Qt Creator and open the `Hi-TEK-qt.pro` file.

An executable named `Hi-TEK-qt` will be built.
...
A warning for people using the *static binary* version of Hi-TEK on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of Hi-TEK is linked against libdb5.3.
...
installed causes Hi-TEK-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
