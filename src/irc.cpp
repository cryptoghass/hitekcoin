...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #Hi-TEKTEST2\r");
            Send(hSocket, "WHO #Hi-TEKTEST2\r");
        } else {
            // randomly join #Hi-TEK00-#Hi-TEK05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #Hi-TEK%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #Hi-TEK%02d\r", channel_number).c_str());
        }
