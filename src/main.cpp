...
CBigNum bnProofOfStakeLegacyLimit(~uint256(0) >> 16); // proof of stake target limit from block #15000 and until 20 June 2013, results with 0,00390625 proof of stake difficulty
CBigNum bnProofOfStakeLimit(~uint256(0) >> 16); // proof of stake target limit since 20 June 2013, equal to 0.03125  proof of stake difficulty
CBigNum bnProofOfStakeHardLimit(~uint256(0) >> 16); // disabled temporarily, will be used in the future to fix minimal proof of stake difficulty at 0.25
uint256 nPoWBase = uint256("0x00000000ffff0000000000000000000000000000000000000000000000000000"); // difficulty-1 target

CBigNum bnProofOfWorkLimitTestNet(~uint256(0) >> 16);

unsigned int nStakeMinAge = 12 * nOneHour; // 0.5 days as zero time weight
...
unsigned int nModifierInterval = 0.1 * nOneHour; // time to elapse before new modifier is computed
...
const string strMessageMagic = "Hi-TEK Signed Message:\n";
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in Hi-TEK
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig) const
{
    ...
    if (fProofOfStake)
    {
        ...
        // Hi-TEK: check proof-of-stake block signature
...
bool CBlock::AcceptBlock()
{
    ...
    int64_t nMaxOffset = 88 * 86400; // 88 days
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Hi-TEK", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "hi-tek";
        CTransaction txNew;
        txNew.nTime = 1521659141;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1521659141;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 3439689; // !fTestNet ? 1575379 : 46534;
